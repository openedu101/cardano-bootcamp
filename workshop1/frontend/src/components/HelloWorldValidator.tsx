...
export const HelloWorldValidator = () => {
  ...
  const unlockAda = async () => {
    try {
      ...
      const tx = await lucid?.newTx().collectFrom(utxos, redeemer).attachSpendingValidator(validator).addSigner(address).complete();
  ...
  return (
    <div>
      <h1 className="text-3xl font-bold mb-6">Hello World Validator</h1>
